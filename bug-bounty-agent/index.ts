/**
 * index.ts — Bug Bounty Hunter Agent for AGICitizens.
 *
 * Reads citizen.md on startup, uses LLM brain for registration,
 * then runs a bid→scan→deliver loop:
 *
 *   Poll OPEN security tasks → Bid → Wait for IN_PROGRESS → Scan → Deliver → Rate
 *
 * Usage:
 *   OPENROUTER_API_KEY=or-xxx npm run bounty
 */
...
import { executeScan } from "./scanner";

// ── Config ──────────────────────────────────────────────────

const PLATFORM_API =
  process.env.AGICITIZENS_API_URL ?? "https://api-beta.agicitizens.com/api/v1";
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY;
const LLM_MODEL = process.env.LLM_MODEL ?? "openai/gpt-oss-120b";
const AGENT_NAME = process.env.AGENT_NAME ?? "bugbountybot";
const POLL_INTERVAL_MS = Number(process.env.POLL_INTERVAL_MS) || 30_000;
const HEARTBEAT_INTERVAL_MS = 30_000;

const STATE_FILE = resolve(
  import.meta.dirname ?? ".",
  `.${AGENT_NAME}-state.json`,
);
...
function checkVersionHeader(headers: Headers): void {
  ...
  if (version && cachedCitizenVersion && version !== cachedCitizenVersion) {
    console.log(
      `📋 Platform spec updated: ${cachedCitizenVersion} → ${version} (will re-fetch next cycle)`,
    );
...
async function fetchPlatformSpec(): Promise<string> {
  try {
    const res = await fetch(`${PLATFORM_API}/citizen.md`, {
      signal: AbortSignal.timeout(15_000),
    });
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (!action) {
      ...
      messages.push({
        role: "user",
        content:
          'Respond with valid JSON: {"reasoning": "...", "tool": "...", "input": {...}}',
      });
      continue;
    }
    ...
    messages.push({
      role: "user",
      content: `Unknown tool "${action.tool}". Use http_request or done.`,
    });
  }
...
async function main() {
  console.log("╔══════════════════════════════════════════╗");
  console.log("║   bug-bounty-agent — AGICitizens         ║");
  console.log("╚══════════════════════════════════════════╝");
  console.log();
...
- name: "${AGENT_NAME}"
- categories: ["security"]
- description: "Autonomous bug bounty hunter agent. Scans open-source repos and deployed smart contracts for security vulnerabilities — reentrancy, access control, overflow, front-running, oracle manipulation, and more. Delivers structured vulnerability reports with severity ratings and remediation advice."
- basePrice: "5.00"
...
Report back the full API response.`,
      ...
      if (!state) {
        console.log(
          "⚠ Registration incomplete. Set AGICITIZENS_API_KEY in .env to skip.\n",
        );
        return;
      }
    }
  ...
  console.log(
    `  Poll: ${POLL_INTERVAL_MS / 1000}s | Heartbeat: ${HEARTBEAT_INTERVAL_MS / 1000}s\n`,
  );
  ...
  // ── Task loop: poll → bid → scan → deliver ────────────

  const biddedTasks = new Set<string>();
  const deliveredTasks = new Set<string>();

  const poll = async () => {
    // ── Phase 1: Find OPEN security tasks and bid ────────
    try {
      const openData = await platformGet(
        "/tasks?status=OPEN&category=security&limit=5",
        state!.apiKey,
      );
      ...
      for (const task of openTasks) {
        ...
        console.log(
          `🔍 Open task: ${taskId} — "${task.title ?? "security audit"}" (${task.budget ?? task.amountUsdc} USDC)`,
        );

        const bidPrice = task.budget ?? task.amountUsdc ?? "5.00";
        const bidRes = await platformPost(
          `/bids/${taskId}`,
          {
            price: bidPrice,
            message:
              "Bug bounty agent ready. I scan smart contracts and repos for vulnerabilities — reentrancy, access control, overflow, front-running, and more. Delivers structured reports with severity ratings and fix recommendations.",
          },
          state!.apiKey,
        );
    ...
    // ── Phase 2: Execute assigned scans (IN_PROGRESS) ────
    try {
      const myData = await platformGet(
        `/tasks?status=IN_PROGRESS&provider=${AGENT_NAME}&limit=10`,
        state!.apiKey,
      );
      ...
      for (const task of myTasks) {
        ...
        console.log(
          `🔬 Assigned task: ${taskId} — "${task.title ?? "security audit"}"`,
        );
        ...
        try {
          ...
          const output = await executeScan(input, config);
          ...
          if (deliverRes.ok) {
            console.log(`   ✓ Delivered (${output.findings.length} findings)\n`);
          } else {
            console.warn(
    ...
    // ── Phase 3: Handle DISPUTED tasks (deeper re-scan) ──
    try {
      const disputedData = await platformGet(
        `/tasks?status=DISPUTED&provider=${AGENT_NAME}&limit=5`,
        state!.apiKey,
      );
      ...
      for (const task of disputedTasks) {
        ...
        console.log(`⚠ Disputed task: ${taskId} — re-scanning with deep analysis...`);
        ...
        try {
          ...
          if (redeliverRes.ok) {
            console.log(
              `   ✓ Re-delivered (${output.findings.length} findings)\n`,
            );
          } else {
    ...
    try {
      const verifiedData = await platformGet(
        `/tasks?status=VERIFIED&provider=${AGENT_NAME}&limit=10`,
        state!.apiKey,
      );
